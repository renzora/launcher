...
import { IconFolder, IconCheck, IconUser } from './icons.jsx';

export default function SettingsPage(props) {
    const [dir, setDir] = createSignal(props.config()?.install_dir || '');
    ...
    async function browseDir() {
        ...
        if (result.path) {
            ...
            await fetchJson(`${API}/config`, {
                method: 'POST',
                body: JSON.stringify({ install_dir: result.path }),
            });
            props.refetchConfig();
            setSaved(true);
        ...
        <div class="flex flex-col gap-6 p-6">
            ...
            <div class="space-y-8 max-w-lg">
                ...
                <div class="space-y-2.5">
                    <div>
                        <label class="text-xs font-semibold text-base-content/70">Install Directory</label>
                        <p class="text-[10px] text-base-content/35 mt-0.5">Where engine versions and games are stored</p>
                    </div>
                    <div class="flex gap-2">
                        ...
                        <button class="btn btn-ghost btn-sm rounded-lg border border-white/[0.06] px-4 text-xs" onClick={browseDir}>
                            Browse
                        </button>
                    </div>
                    <Show when={saved()}>
                        <div class="flex items-center gap-1.5 text-success text-[11px]">
                            <IconCheck size={14} />
                            Directory saved
                        </div>
                    </Show>
                ...
                <div class="space-y-3">
                    ...
                    <Show
                        when={props.config()?.logged_in}
                        fallback={
                            <button
                                class="btn btn-primary btn-sm rounded-lg w-full"
                    ...
                    >
                        <div class="rounded-xl border border-white/[0.04] bg-base-200/30 overflow-hidden">
                            <div class="flex items-center gap-3 px-4 py-3">
                                ...
                                <div class="flex items-center gap-1.5 text-success text-[10px]">
                                    <IconCheck size={12} />
                                    Connected
                                </div>
                            </div>
                            <div class="border-t border-white/[0.04] px-4 py-2.5">
                                <button
                                    class="btn btn-ghost btn-xs text-error/60 hover:text-error hover:bg-error/10 rounded-lg"
                                    onClick={signOut}
                                >
                                    Sign out
                                </button>
