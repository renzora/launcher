...
import { API, UPDATER_API, fetchJson, formatSize, formatDate } from './api.jsx';
...
function injectEngineStyles() {
    ...
    style.textContent = `
        ...
        .template-card:hover {
            ...
            box-shadow: 0 4px 20px rgba(139, 106, 174, 0.06);
        }
        .template-card {
...
// Particle canvas — connected dots that react to mouse
function ParticleCanvas() {
    ...
    onMount(() => {
        ...
        function resize() {
            w = canvasRef.width = canvasRef.offsetWidth;
            h = canvasRef.height = canvasRef.offsetHeight;
        }
        resize();
        ...
        function draw() {
            ...
            for (let i = 0; i < particles.length; i++) {
                ...
                ctx.fillStyle = 'rgba(139, 106, 174, 0.5)';
                ctx.fill();
                ...
                for (let j = i + 1; j < particles.length; j++) {
                    ...
                    if (d < 15000) {
                        ...
                        ctx.strokeStyle = `rgba(139, 106, 174, ${alpha})`;
                        ctx.lineWidth = 0.5;
                        ctx.stroke();
                    }
                }
...
// Templates are cross-platform — show all since you export to other platforms
...
export default function EnginePage(props) {
    ...
    return (
        <div class="flex flex-col gap-0">
            ...
            <div class="px-6 pb-8">
                {/* Editor Download — current OS only */}
                    ...
                    <div class="mb-10">
                        <h2 class="text-sm font-semibold mb-4 flex items-center gap-2">
                            <div class="w-6 h-6 rounded-lg bg-accent/10 flex items-center justify-center">
                                <IconDownload size={13} class="text-accent" />
                            </div>
                            Editor
                        </h2>
                        <EditorCard
                            release={latestRelease()}
                            isInstalled={() => isVersionInstalled(latestRelease().version)}
                    ...
                    <div class="mb-10">
                        <h2 class="text-sm font-semibold mb-1 flex items-center gap-2">
                            <div class="w-6 h-6 rounded-lg bg-primary/10 flex items-center justify-center">
                                <IconWorld size={13} class="text-primary" />
                            </div>
                            Export Templates
                        </h2>
                ...
                <div class="mb-10">
                    <h2 class="text-sm font-semibold mb-4 flex items-center gap-2">
                        <div class="w-6 h-6 rounded-lg bg-success/10 flex items-center justify-center">
                            <IconTerminal2 size={13} class="text-success" />
                        </div>
                        Other Options
                    </h2>
                    <div class="grid grid-cols-2 gap-3">
                        <a href="https://github.com/renzora/engine" target="_blank"
                            class="option-card group p-4 bg-base-200/20 border border-white/[0.04] rounded-xl hover:border-white/[0.08] hover:bg-base-200/30 transition-all flex items-center gap-3 anim-fade-up anim-delay-5">
                            <div class="option-icon w-9 h-9 rounded-xl bg-base-300/50 flex items-center justify-center shrink-0">
                                <IconTerminal2 size={18} class="text-base-content/40" />
                            </div>
                            <div class="flex-1 min-w-0">
                                <div class="text-xs font-semibold mb-0.5 group-hover:text-accent transition-colors">Build from source</div>
                                <div class="text-[10px] text-base-content/30">Clone the repo and compile with Cargo.</div>
                            </div>
                            <IconArrowRight size={14} class="text-base-content/20 group-hover:text-accent transition-colors shrink-0" />
                        </a>
                        <a href="https://github.com/renzora/engine/releases" target="_blank"
                            class="option-card group p-4 bg-base-200/20 border border-white/[0.04] rounded-xl hover:border-white/[0.08] hover:bg-base-200/30 transition-all flex items-center gap-3 anim-fade-up anim-delay-6">
                            <div class="option-icon w-9 h-9 rounded-xl bg-base-300/50 flex items-center justify-center shrink-0">
                                <IconGitBranch size={18} class="text-base-content/40" />
                            </div>
                            <div class="flex-1 min-w-0">
                                <div class="text-xs font-semibold mb-0.5 group-hover:text-accent transition-colors">All releases</div>
                                <div class="text-[10px] text-base-content/30">Browse all versions on GitHub.</div>
                            </div>
                            <IconArrowRight size={14} class="text-base-content/20 group-hover:text-accent transition-colors shrink-0" />
                        </a>
                    </div>
                ...
                <Show when={olderReleases().length > 0}>
                    <div>
                        <button
                            class="flex items-center gap-2 text-xs font-medium text-base-content/40 hover:text-base-content/60 transition-colors mb-3"
                            ...
                            <div class="space-y-2">
                                <For each={olderReleases()}>
                                    {(release) => (
                                        <OlderVersionRow
                                            release={release}
                                            isInstalled={() => isVersionInstalled(release.version)}
                                            refetchInstalled={props.refetchInstalled}
                                        />
                                    )}
                                </For>
...
function UpdateBanner() {
    const [updateInfo] = createResource(() => fetchJson(`${UPDATER_API}/check`).catch(() => null));
    const [updating, setUpdating] = createSignal(false);
    const [result, setResult] = createSignal(null);

    async function performUpdate() {
        setUpdating(true);
        const r = await fetchJson(`${UPDATER_API}/update`, { method: 'POST' });
        setResult(r);
        setUpdating(false);
    }
...
function EditorCard(props) {
    // Find the editor asset for current OS
    ...
    async function install() {
        ...
        await fetchJson(`${API}/install`, {
            method: 'POST',
            body: JSON.stringify({ version: props.release.version, assets: [asset().name] }),
        });
        setInstalling(false);
        setDone(true);
    ...
    async function launch() {
        const result = await fetchJson(`${API}/launch`, {
            method: 'POST',
            body: JSON.stringify({ version: props.release.version }),
        });
        if (result.error) {
    ...
    return (
        <div class={`relative rounded-xl border engine-card anim-fade-up anim-delay-3 ${
            available()
                ? 'bg-base-200/20 border-white/[0.04] hover:border-accent/30'
                : 'bg-base-200/10 border-white/[0.02] opacity-40'
        }`}>
            <div class="flex items-center gap-4 p-5">
                ...
                <div class="flex-1">
                    ...
                    <Show when={available()}>
                        <p class="text-[10px] text-base-content/20 mt-0.5">{asset().name} · {formatSize(asset().size)}</p>
                    </Show>
                </div>
                <div class="shrink-0 flex items-center gap-1.5">
                    <Show when={available()} fallback={
                        <span class="px-5 py-2.5 rounded-xl text-xs font-medium bg-base-300/30 text-base-content/20">
                            Coming soon
                        </span>
                    }>
                        <Show when={isInstalled()} fallback={
                            <button
                                class="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl text-sm font-medium bg-primary text-primary-content hover:brightness-110 transition-all"
                                onClick={install}
                                disabled={installing()}
                            >
                                <Show when={!installing()} fallback={<span class="loading loading-spinner loading-xs" />}>
                            ...
                            <button
                                class="launch-btn inline-flex items-center gap-2 px-5 py-2.5 rounded-xl text-sm font-medium bg-success text-success-content hover:brightness-110 transition-all"
                                onClick={launch}
                            >
                            ...
                            <div class="relative">
                                <button
                                    class="p-2 rounded-lg text-base-content/30 hover:text-base-content/60 hover:bg-white/[0.04] transition-all"
                                    onClick={toggleMenu}
                                >
                                ...
                                <Show when={menuOpen()}>
                                    <div class="absolute right-0 top-full mt-1 z-50 w-40 py-1 rounded-xl bg-base-200 border border-white/[0.08] shadow-xl">
                                        <button
                                            class="w-full flex items-center gap-2 px-3 py-2 text-xs text-error/70 hover:text-error hover:bg-error/10 transition-colors"
                                            onClick={uninstall}
                                        >
                                            <IconTrash size={14} />
                                            Remove
...
function TemplateCard(props) {
    ...
    return (
        <div
            class={`p-3.5 rounded-xl border flex flex-col items-center gap-2 template-card anim-fade-up ${
                available()
                    ? 'bg-base-200/20 border-white/[0.04] hover:border-primary/20 hover:bg-base-200/30'
                    : 'bg-base-200/10 border-white/[0.02] opacity-40'
            }`}
            style={`animation-delay: ${(props.index || 0) * 60 + 200}ms`}
        >
            ...
            <Show when={available()} fallback={
                <span class="text-[10px] text-base-content/20">Coming soon</span>
            }>
                <Show when={isInstalled()} fallback={
                    <button
                        class="w-full inline-flex items-center justify-center gap-1.5 px-3 py-1.5 rounded-lg text-[11px] font-medium bg-primary text-primary-content hover:brightness-110 transition-colors"
                        onClick={install}
                        disabled={installing()}
                    >
                        <Show when={!installing()} fallback={<span class="loading loading-spinner loading-xs" />}>
                    ...
                    <div class="w-full inline-flex items-center justify-center gap-1 px-3 py-1.5 rounded-lg text-[11px] font-medium bg-success/15 text-success">
...
function OlderVersionRow(props) {
    ...
    async function install() {
        ...
        await fetchJson(`${API}/install`, {
            method: 'POST',
            body: JSON.stringify({ version: props.release.version, assets: [a.name] }),
        });
        setInstalling(false);
        setDone(true);
        props.refetchInstalled();
        ...
        <div class="flex items-center justify-between px-4 py-3 rounded-xl border border-white/[0.04] bg-base-200/15 hover:border-white/[0.06] transition-colors">
            <div class="flex items-center gap-3">
                <div class={`w-8 h-8 rounded-lg flex items-center justify-center text-xs font-bold ${
                    isInstalled() ? 'bg-primary/15 text-primary' : 'bg-base-300/30 text-base-content/25'
                }`}>
                ...
                <div>
                    <div class="flex items-center gap-2">
                        ...
                        <Show when={props.release.prerelease}>
                            <span class="text-[9px] px-1.5 py-0.5 rounded-full bg-warning/15 text-warning font-semibold">pre-release</span>
                        </Show>
                        <Show when={isInstalled()}>
                            <span class="text-[9px] px-1.5 py-0.5 rounded-full bg-success/15 text-success font-semibold">installed</span>
                        </Show>
                    </div>
            ...
            <div class="flex items-center gap-1">
                <Show when={isInstalled()} fallback={
                    ...
                    <button class="btn btn-primary btn-xs rounded-lg gap-1" onClick={launch}>
                        <IconPlayerPlay size={12} /> Launch
                    </button>
                    <div class="relative">
                        <button class="p-1.5 rounded-lg text-base-content/25 hover:text-base-content/50 hover:bg-white/[0.04] transition-all" onClick={toggleMenu}>
                            <IconDotsVertical size={14} />
                        </button>
                        <Show when={menuOpen()}>
                            <div class="absolute right-0 top-full mt-1 z-50 w-36 py-1 rounded-xl bg-base-200 border border-white/[0.08] shadow-xl">
                                <button class="w-full flex items-center gap-2 px-3 py-2 text-xs text-error/70 hover:text-error hover:bg-error/10 transition-colors" onClick={uninstall}>
