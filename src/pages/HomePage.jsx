        ...
        <div class="flex flex-col gap-6 p-6">
                ...
                <div class="relative px-8 py-10 flex items-center justify-between">
                    <div class="max-w-md">
                        ...
                        <div class="flex gap-3">
                            <button class="btn btn-primary btn-sm rounded-lg gap-1.5 px-5" onClick={() => props.setActive('store')}>
                                Browse Store <IconArrowRight size={14} />
                            </button>
                            <button class="btn btn-ghost btn-sm rounded-lg border border-white/[0.06] px-5" onClick={() => props.setActive('engine')}>
                                Get Engine
                            </button>
                        </div>
                    </div>
            ...
            <div>
                <div class="flex items-center justify-between mb-3">
                    ...
                    <button class="text-[11px] text-base-content/40 hover:text-primary flex items-center gap-1 transition-colors" onClick={() => props.setActive('store')}>
                ...
                <div class="grid grid-cols-2 xl:grid-cols-4 gap-3">
                    <For each={FEATURED_GAMES}>
                        {(game) => (
                            <button class="group relative rounded-xl overflow-hidden border border-white/[0.04] hover:border-primary/30 transition-all hover:scale-[1.02] active:scale-[0.98]" onClick={() => props.setActive('store')}>
            ...
            <div class="grid grid-cols-3 gap-3">
                <button class="flex items-center gap-3 px-4 py-3 rounded-xl bg-base-200/30 border border-white/[0.04] hover:border-primary/20 transition-all group" onClick={() => props.setActive('library')}>
                    <div class="w-9 h-9 rounded-lg bg-primary/10 flex items-center justify-center text-primary group-hover:bg-primary/20 transition-colors"><IconPlay size={16} /></div>
                    <div class="text-left"><div class="text-xs font-semibold">My Library</div><div class="text-[10px] text-base-content/35">Launch your games</div></div>
                </button>
                <button class="flex items-center gap-3 px-4 py-3 rounded-xl bg-base-200/30 border border-white/[0.04] hover:border-primary/20 transition-all group" onClick={() => props.setActive('engine')}>
                    <div class="w-9 h-9 rounded-lg bg-secondary/10 flex items-center justify-center text-secondary group-hover:bg-secondary/20 transition-colors"><IconDownload size={16} /></div>
                    <div class="text-left"><div class="text-xs font-semibold">Engine</div><div class="text-[10px] text-base-content/35">Manage versions</div></div>
                </button>
                <button class="flex items-center gap-3 px-4 py-3 rounded-xl bg-base-200/30 border border-white/[0.04] hover:border-primary/20 transition-all group" onClick={() => props.setActive('store')}>
                    <div class="w-9 h-9 rounded-lg bg-accent/10 flex items-center justify-center text-accent group-hover:bg-accent/20 transition-colors"><IconStarFilled size={16} /></div>
                    <div class="text-left"><div class="text-xs font-semibold">Store</div><div class="text-[10px] text-base-content/35">Discover content</div></div>
                </button>
            </div>
                    ...
                    <For each={NEWS_ITEMS}>
                        {(item) => (
                            <div class="flex items-start gap-4 px-4 py-3 rounded-xl bg-base-200/20 border border-white/[0.03] hover:border-white/[0.06] transition-colors">
                                <div class={`shrink-0 mt-0.5 w-2 h-2 rounded-full ${item.type === 'Engine' ? 'bg-primary' : item.type === 'Game' ? 'bg-secondary' : 'bg-accent'}`} />
